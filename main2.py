...
# default = 1 → laptop camera  defult = 3 → usb camera
...
def main():
    ...
    while True:
        ...
        # 　画像保存
        if cropping_image is not None:
            datetime = dt.now()
            tester = datetime.strftime('%Y_%m_%d_%H_%M_%S')
            image = 'image/' + tester + '.jpg'
            cv.imwrite(image, cropping_image)
            print('image/' + tester + '.jpg')
            time.sleep(1)
            cropping_image = None
